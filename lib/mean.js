...
Meanio.prototype.app = function(options, callback) {
    ...
    var defaultConfig = require(options.config ? options.config + '/config' : process.cwd() + '/config/config');

    var database = mongoose.connect(defaultConfig.db || '');
    ...
    this.config = new Config(options.config, function(err, data) {
        ...
        function bootstrapModels() {
            // Bootstrap models
            util.walk(process.cwd() + '/server', 'model', null, function(path) {
                require(path);
            });
        }
        ...
        require(options.config ? options.config + '/passport' : process.cwd() + '/config/passport')(passport);
        ...
        // Express settings
        var app = express();
        require(process.cwd() + '/config/express')(app, passport, database);
...
function Config(path, callback) {
    ...
    configPath = (path || process.cwd() + '/config/config');
